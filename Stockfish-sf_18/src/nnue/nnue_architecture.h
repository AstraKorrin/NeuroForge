    ...
    std::int32_t propagate(const TransformedFeatureType* transformedFeatures) const {
        struct alignas(CacheLineSize) Buffer {
            ...
            alignas(CacheLineSize) typename decltype(ac_sqr_0)::OutputType
              concat_buffer[ceil_to_multiple<IndexType>(
                FC_0_OUTPUTS * 2 + FC_1_OUTPUTS * 2, 32)];
        ...
        std::int32_t fwdOut = buffer.fc_2_out[0]
                            + buffer.fc_0_out[FC_0_OUTPUTS - 2]
                            - buffer.fc_0_out[FC_0_OUTPUTS - 1];
        constexpr std::int64_t multiplier = 600 * OutputScale;
        constexpr std::int64_t denominator =
          static_cast<std::int64_t>(128) * (1 << WeightScaleBits) * 2;
        std::int32_t outputValue =
          static_cast<std::int32_t>((static_cast<std::int64_t>(fwdOut) * multiplier)
                                    / denominator);
